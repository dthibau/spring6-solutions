...
import org.formation.service.UserWithOperation;
...
public class WebclientApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// Get all
		Flux<UserWithOperation> accounts = client.get().uri("/accounts").accept(MediaType.APPLICATION_JSON).exchange()
				.flatMapMany(r -> r.bodyToFlux(UserWithOperation.class));

		accounts = client.get().uri("/accounts").accept(MediaType.APPLICATION_JSON).retrieve().bodyToFlux(UserWithOperation.class);
